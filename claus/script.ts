...
class Car {
  ...
  ps: number;
  img: string;
  constructor(model:string, built: number, kilometres: number, ps: number, img: string){
    this.model = model;
    this.built = built;
...
class Elektro extends Car {
  ...
  constructor(model:string, built: number, kilometres: number, ps: number, img: string, battery: string, reach: string){
...
for (let each of electroCars) {
  (document.getElementById("elektro") as HTMLElement).innerHTML += `
  <div class="col">
...
class Hybrid extends Car {
  ...
  constructor(model:string, built: number, kilometres: number, ps: number, img: string, battery: string, plugin: boolean){
    super(model,built,kilometres,ps,img)
    this.battery = battery;
    this.plugin = plugin;
...
function printHybrid() {
  for (let each of hybridCars) {
    (document.getElementById("hybrid") as HTMLElement).innerHTML += `
    <div class="col">
    <div class="card h-100 p-2 text-align-center" style="width: 18rem;">
      <img src="${each.img}" class="card-img-top" alt="...">
      <div class="card-body">
...
class SUV extends Car {
  ...
  constructor(model:string, built: number, kilometres: number, ps: number, img: string, loud: boolean, idiotic: boolean){
    super(model,built,kilometres,ps,img)
    this.loud = loud;
...
function printSuv() {
  for (let each of suvCars) {
    (document.getElementById("suv") as HTMLElement).innerHTML += `
    <div class="col">
